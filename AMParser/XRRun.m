...
- (NSString *)formattedSample:(NSUInteger)index setTarget:(NSString *)target
{
    ...
    [result appendFormat:@"Process: %@ ", target];
    
    NSArray *processData = data[@"Processes"];
    for (NSDictionary *process in processData) {
        if ([process[@"Command"] isEqualToString:target]) {
...
- (NSString *)parseTracefile:(NSString *)target
{
	...
	{
		[result appendFormat:@"Sample %u: %@\n", (unsigned int)i, [self formattedSample:i setTarget:target]];
	}
